...
export class Outlet {
  ...
  async refreshStatus() {
    try {
      this.platform.log.debug('Plug - Reading', `${this.platform.config.device.deviceUrl}/status`);
      const deviceStatus = (
        await this.platform.axios.get(`http://${this.platform.config.device.deviceUrl}/config`)
      ).data;
      ...
      this.platform.log.error(
        `Plug - Failed to refresh status of ${this.device.name}`,
 ...
 */
  async pushChanges() {
    ...
    const push = await this.platform.axios.post(`http://${this.platform.config.device.deviceUrl}/config`, payload);
