...
export default class {
  ...
  applyUpdate(data) {
    // {"id":"XX","status":1,"onoff":0,"lastActiveTime":${ts},"waterLeak":{"latestWaterLeak":0,"latestSampleTime":0}}
    // Sensor should be based on `data.waterLeak.latestWaterLeak` - 1 is a leak, 0 is no leak
    try {
      this.accessory.logWarn(`LOGGING TO COPY ${JSON.stringify(data)}`);
    } catch (err) {
