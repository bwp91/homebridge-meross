...
import colourUtils from '../utils/colour-utils.js';
...
export default class {
  ...
  async internalLightColourUpdate(value) {
    try {
      ...
      await this.queue.add(async () => {
        ...
        const [r, g, b] = colourUtils.hs2rgb(value, saturation);
        ...
        const payload = {
          light: {
            capacity: 1,
            channel: 0,
            rgb: colourUtils.hk2mrRGB(r, g, b),
            onoff: 1,
            luminance: 100,
          },
        };
        ...
        if (!this.cacheLightState) {
          ...
          if (this.enableLogging) {
            this.log('[%s] current light state [on].', this.name);
          }
          this.cacheLightBright = 100;
  ...
  applyUpdate(data) {
    ...
    if (data.spray && data.spray[0] && platformFuncs.hasProperty(data.spray[0], 'mode')) {
    ...
    if (data.light) {
      if (platformFuncs.hasProperty(data.light, 'onoff')) {
      ...
      if (platformFuncs.hasProperty(data.light, 'rgb') && !this.ignoreNextMQTT) {
        const [r, g, b] = colourUtils.mr2hkRGB(data.light.rgb);
        const [newHue, newSat] = colourUtils.rgb2hs(r, g, b);
