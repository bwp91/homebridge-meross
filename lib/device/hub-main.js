    ...
    try {
      ...
      return await this.queue.add(async () => {
        ...
        if (data.all) {
          if (
            ...
            data.all.digest.hub.subdevice.forEach(subdevice => {
              ...
              if (this.funcs.hasProperty(subdevice, 'scheduleBMode')) {
        ...
        if (this.mtsList.length > 0) {
          const payload = { all: [] }
          this.mtsList.forEach(id => payload.all.push({ id: id }))
          ...
          // Log the received data
          if (this.enableDebugLogging) {
            this.log('[%s] %s: %s.', this.name, this.lang.incPoll, JSON.stringify(res2.data))
          }
          ...
          if (data2.all && Array.isArray(data2.all)) {
            data2.all.forEach(entry => {
              ...
              const toReturn = {}
              if (entry.togglex && this.funcs.hasProperty(entry.togglex, 'onoff')) {
                toReturn.state = entry.togglex.onoff
              }
              if (entry.temperature) {
                if (this.funcs.hasProperty(entry.temperature, 'room')) {
                  toReturn.currTemperature = entry.temperature.room / 10
                }
                if (this.funcs.hasProperty(entry.temperature, 'currentSet')) {
                  toReturn.targTemperature = entry.temperature.currentSet / 10
                }
                if (this.funcs.hasProperty(entry.temperature, 'openWindow')) {
                  toReturn.openWindow = entry.temperature.openWindow
                }
              }
              if (entry.mode && this.funcs.hasProperty(entry.mode, 'state')) {
    ...
    try {
      ...
      if (data.togglex && Array.isArray(data.togglex)) {
        data.togglex.forEach(entry => {
          ...
          const toReturn = {}
          if (this.funcs.hasProperty(entry, 'onoff')) {
      ...
      if (data.temperature && Array.isArray(data.temperature)) {
        data.temperature.forEach(entry => {
          ...
          const toReturn = {}
          if (this.funcs.hasProperty(entry, 'currentSet')) {
            toReturn.targTemperature = entry.currentSet / 10
          }
          if (this.funcs.hasProperty(entry, 'room')) {
            toReturn.currTemperature = entry.room / 10
          }
          if (this.funcs.hasProperty(entry, 'openWindow')) {
      ...
      if (data.mode && Array.isArray(data.mode)) {
        data.mode.forEach(entry => {
          ...
          const toReturn = {}
          if (this.funcs.hasProperty(entry, 'state')) {
