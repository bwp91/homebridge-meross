...
module.exports = class deviceHubMain {
  ...
  receiveUpdate (params) {
    try {
      ...
      if (params.payload && params.payload.temperature && params.payload.temperature.id) {
        // Check whether the homebridge accessory this relates to exists
        const subAcc = this.devicesInHB.get(
          this.platform.api.hap.uuid.generate(
            this.accessory.context.serialNumber + params.payload.temperature.id
          )
        )
        ...
        const toReturn = {}
        if (this.funcs.hasProperty(params.payload.temperature, 'currentSet')) {
          toReturn.targTemperature = params.payload.temperature.currentSet / 10
        }
        if (this.funcs.hasProperty(params.payload.temperature, 'room')) {
          toReturn.currTemperature = params.payload.temperature.room / 10
        }
