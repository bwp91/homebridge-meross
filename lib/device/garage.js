...
module.exports = class deviceGarage {
  ...
  async internalCloudTargetUpdate (value) {
    this.log.warn('[%s] this feature is not yet supported.')
  }
  ...
  async requestLocalUpdate () {
    try {
      ...
      if (
        ...
        let newCurrent
        for (let i = 0; i < data.payload.all.digest.garageDoor.length; i++) {
          if (data.payload.all.digest.garageDoor[i].channel === this.channel) {
            isOpen = data.payload.all.digest.garageDoor[i].open
          }
        }
        if (isOpen) {
