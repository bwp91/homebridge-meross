      ...
      return await this.queue.add(async () => {
        ...
        if (newState !== this.cacheState) {
          ...
          if (this.enableLogging) {
            this.log('[%s] %s [%s].', this.name, 'current state', this.cacheState ? 'on' : 'off')
          }
        }
        if (newCool !== this.cacheCool) {
          this.cacheCool = newCool
          if (this.enableLogging) {
            this.log('[%s] %s [%s].', this.name, 'current cooling', this.cacheCool ? 'on' : 'off')
          }
        }
        this.service.updateCharacteristic(
          this.hapChar.CurrentHeaterCoolerState,
          value === 1 ? (this.cacheCool ? 3 : 1) : 0
        )
      })
    } catch (err) {
  ...
  async internalTargetTempUpdate (value) {
    try {
      ...
      return await this.queue.add(async () => {
        ...
        if (this.enableLogging) {
          this.log('[%s] %s [%s°C].', this.name, 'current target', value)
        }
        if (!this.cacheState) {
          return
        }
  ...
  async getTemperature () {
    try {
      ...
      if (newTemp && newTemp !== this.cacheTemp) {
        ...
        if (this.enableLogging) {
          this.log('[%s] %s [%s°C].', this.name, 'current temperature', this.cacheTemp)
        }
