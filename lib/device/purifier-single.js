...
export default class {
    ...
    try {
      ...
      await this.queue.add(async () => {
        ...
        if (this.enableLogging) {
          this.log('[%s] current state [%s].', this.name, value ? 'purifying' : 'off');
        }
      });
    } catch (err) {
  ...
  applyUpdate(data) {
    ...
    if (platformFuncs.hasProperty(data, 'onoff')) {
    ...
    if (platformFuncs.hasProperty(data, 'power')) {
    ...
    if (platformFuncs.hasProperty(data, 'voltage')) {
  ...
  async setupPowerReadings() {
    try {
      ...
      const eText = platformFuncs.parseError(err, ['no data on initial run']);
      if (this.enableDebugLogging) {
