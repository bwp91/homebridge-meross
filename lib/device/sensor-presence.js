...
export default class {
  ...
  applyUpdate(data) {
    // Example input for light sensor
    // {
    //   "light": [
    //     {
    //       "value": 185,
    //       "timestamp": 1721487473
    //     }
    //   ]
    // }
    ...
    // Example input for presence sensor
    // {
    //   "presence": [
    //     {
    //       "times": 1,
    //       "distance": 3060,
    //       "value": 2,
    //       "timestamp": 1721487703
    //     }
    //   ]
    // }
    // Currently unsure on what the integer values map to in the meross statuses
    // In the data I have seen values [1, 2, 3] and statuses [Presence, Minor Motion, Motion, Moving Away, Absence]
    // I believe Presence maps to 2
    if (data.presence?.[0]?.value) {
      // Check against the cache and update HomeKit and the cache if needed
      const newOccupancy = data.presence[0].value === 2 ? 1 : 0
      if (this.cacheOccupancy !== newOccupancy) {
        ...
        this.accessory.log(`${platformLang.curOcc} [${newOccupancy ? 'yes' : 'no'}]`)
      }
    }
  }
}
