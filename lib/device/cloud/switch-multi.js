...
module.exports = class deviceCloudSwitchMulti {
  constructor (platform, accessory) {
    ...
    Object.values(accessory.context.channels).forEach(channel => {
      ...
      // Find or add the switch service
      let service =
        this.accessory.getService(channel.devName) ||
        this.accessory.addService(this.hapServ.Switch, channel.devName, channel.devName)

      // Check the service is a switch service
      if (service.constructor.name !== 'Switch') {
        this.accessory.removeService(service)
        service = this.accessory.addService(this.hapServ.Switch, channel.devName, channel.devName)
      }
    ...
    this.accessory.services.forEach(service => {
      // Remove any outlet services the accessory has
      if (service.constructor.name === 'Outlet') {
        this.accessory.removeService(service)
        return
      }

      // We now only concentrate on the switch services
      if (service.constructor.name !== 'Switch') {
        return
      }
  ...
  externalUpdate (namespace, payload) {
    try {
      ...
      toUpdate.forEach(item => {
        ...
        const service = this.accessory.getService(serviceName)
        const newState = item.onoff === 1

        // Don't continue if the service doesn't exist or state is the same as before
        if (!service || newState === this.cacheStates[serviceName]) {
          return
        }
