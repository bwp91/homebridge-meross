...
export default class {
  constructor(platform, accessory) {
    ...
    // Add a window covering service
    this.service = this.accessory.getService(this.hapChar.WindowCovering)
  ...
  async internalLocationUpdate(value) {
    try {
      ...
      await this.queue.add(async () => {
        ...
        this.accessory.log(`${platformLang.curTarg} [${value}%]`);
      });
    } catch (err) {
  ...
  applyUpdate(data) {
    ...
    if (hasProperty(data, 'onoff')) {
      const newTarg = data.onoff;

      // Check against the cache and update HomeKit and the cache if needed
      if (this.cacheTarget !== newTarg) {
        this.service.updateCharacteristic(this.hapChar.On, newTarg);
        this.cacheTarget = newTarg;

        this.accessory.log(`${platformLang.curTarg} [${newTarg}]`);
      }
    }
  }
}
