...
export default class {
  ...
  applyUpdate(data) {
    this.accessory.log(`applyUpdate: ${JSON.stringify(data)}`);
    this.accessory.log(`caches: pos:${this.cachePos} targ:${this.cacheTarg} state:${this.cacheState}`);
        ...
        switch (this.cacheState) {
          ...
          case 1: {
            if (!this.isFromHomeKit) {
              // Device is opening, so hacky set the target position to 100%
              this.cacheTarg = 100;
              this.service.updateCharacteristic(this.hapChar.TargetPosition, this.cacheTarg);
            }
          ...
          case 2: {
            if (!this.isFromHomeKit) {
              // Device is opening, so hacky set the target position to 0%
              this.cacheTarg = 0;
              this.service.updateCharacteristic(this.hapChar.TargetPosition, this.cacheTarg);
            }
