...
export default class {
    constructor(platform, accessory) {
        ...
        for (let i = 0; i++ < accessory.context.channelCount;) {
    ...
    applyUpdate(data) {
        ...
        for (let i = 0; i++ < this.accessory.context.channelCount;) {
            const outletService = this.accessory.getServiceByUUIDAndSubType(this.hapServ.Outlet, `outlet-${i}`);
            if (outletService.getCharacteristic(this.hapChar.On).value) {
                primaryState = true;
                break;
            }
        }
        const hapChar = this.accessory.getService(this.hapServ.Outlet).getCharacteristic(this.hapChar.On);
        if (hapChar.value !== primaryState) {
            hapChar.updateValue(primaryState);
            this.accessory.log(`${platformLang.curState} [${primaryState ? 'on' : 'off'}]`);
        }
    }
}