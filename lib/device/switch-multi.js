        ...
        case 0: {
          ...
          for (let i = 1; i < this.accessory.context.channelCount; i++) {
            ...
            if (subAcc) {
              ...
              if (curState !== value) {
                subAcc.getService(this.hapServ.Switch).updateCharacteristic(this.hapChar.On, value)
          ...
          for (let i = 1; i <= this.accessory.context.channelCount; i++) {
            ...
            if (subAcc) {
              if (i === this.accessory.context.channel) {
                ...
                if (
                  subAcc.getService(this.hapServ.Switch).getCharacteristic(this.hapChar.On).value
                ) {
                  primaryState = true
                }
              }
  ...
  async internalLocalStateUpdate (value) {
    try {
      ...
      return await this.queue.add(async () => {
        ...
        this.service.getCharacteristic(this.hapChar.On).value = value
        if (this.enableLogging) {
          this.log('[%s] current state [%s].', this.name, value ? 'on' : 'off')
        }
  ...
  async requestCloudUpdate () {
    try {
      ...
      if (!this.platform.hideMasters.includes(this.accessory.context.serialNumber)) {
        ...
        const priAcc = this.devicesInHB.get(uuidPri)
        if (priAcc) {
          const curState = priAcc.getService(this.hapServ.Switch).getCharacteristic(this.hapChar.On)
            .value
          if (primaryState !== curState) {
            priAcc
              .getService(this.hapServ.Switch)
