...
module.exports = class deviceBaby {
  constructor (platform, accessory) {
    ...
    this.cacheVolume = this.speakerService.getCharacteristic(this.hapChar.Volume).value
  ...
  async internalVolumeUpdate (value) {
    this.log.error('SET VolumeSelector: ', value, value === 0 ? 'up' : 'down')
    this.log.warn(
      'current volume:',
      this.speakerService.getCharacteristic(this.hapChar.Volume).value
    )

    // Need special check for value === 0 to set the device off if this.cacheState is ON
  }
  ...
  applyUpdate (data) {
    ...
    if (this.funcs.hasProperty(data, 'volume')) {
      ...
      if (data.volume !== this.cacheVolumeMR) {
        ...
        this.speakerService.updateCharacteristic(this.hapChar.Volume, this.cacheVolume)
        if (this.enableLogging) {
          this.log('[%s] current volume [%s%].', this.name, this.cacheVolume)
        }
      }
    }
