...
module.exports = class deviceLightbulb {
  constructor (platform, accessory) {
    ...
    this.colourModels = ['MSL100', 'MSL120', 'MSL120D', 'MSL320', 'MSL320M', 'MSL420']
    ...
    if (this.colourModels.includes(this.accessory.context.model)) {
  ...
  applyUpdate (data) {
    if (this.accessory.context.model === 'MSS1101') {
      if (this.funcs.hasProperty(data.toggle, 'onoff')) {
        // newState is given as 0 or 1 -> convert to bool for HomeKit
        const newState = data.toggle.onoff === 1
      ...
      if (data.light) {
        if (data.light.capacity === 4 && this.funcs.hasProperty(data.light, 'luminance')) {
        ...
        if (
          data.light.capacity === 1 &&
          this.funcs.hasProperty(data.light, 'rgb') &&
          data.light.rgb !== -1
        ) {
          const [r, g, b] = this.colourUtils.mr2hk(data.light.rgb)
        ...
        if (
          data.light.capacity === 2 &&
          this.funcs.hasProperty(data.light, 'temperature') &&
          data.light.temperature !== -1
        ) {
