...
const hk2mrRGB = (r, g, b) => (r << 16) + (g << 8) + b; // eslint-disable-line no-bitwise

const hk2mrCT = (val) => {
  let mrVal = val - 140;
  mrVal = 360 - mrVal;
  mrVal = Math.round(mrVal / 3.6);
  mrVal = mrVal === 0 ? 1 : mrVal;
  return mrVal;
};

const mr2hkRGB = (val) => {
  const r = (val & 0xff0000) >> 16; // eslint-disable-line no-bitwise
  const g = (val & 0x00ff00) >> 8; // eslint-disable-line no-bitwise
  const b = val & 0x0000ff; // eslint-disable-line no-bitwise
  return [r, g, b];
};
