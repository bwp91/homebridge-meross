...
export default class {
          ...
          if (Array.isArray(val) && val.length > 0) {
            val.forEach((x) => {
              if (
                !x.serialNumber
                || !x.name
                || (((config.connection === 'local' && !x.connection) || x.connection === 'local') && !x.deviceUrl && !x.model)
              ) {
                logIgnoreItem(key);
                return;
              }
  ...
  applyAccessoryLogging(deviceConf, accessory) {
  ...
  async initialiseDevice(device) {
    try {
      ...
      if (platformConsts.models.switchSingle.includes(device.model)) {
        ...
        this.applyAccessoryLogging(deviceConf, accessory);
        ...
        device.channels.forEach((channel, index) => {
          ...
          this.applyAccessoryLogging(deviceConf, subAcc);
        ...
        this.applyAccessoryLogging(deviceConf, accessoryLight);
