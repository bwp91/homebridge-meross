                ...
                switch (k) {
                  ...
                  case 'serialNumber':
                    if (typeof v !== 'string' || v === '') {
                      ...
                      const formatted = v.toLowerCase().replace(/[^a-z0-9]+/g, '')
                      if (formatted.length === 32) {
                        // Check if another entry in deviceConf already has this ID (no duplicates)
                        if (Object.keys(this.deviceConf).includes(formatted)) {
                          logDuplicate(key + '.' + id)
                          isValid = false
                        } else {
                          entry[k] = formatted
    ...
    try {
      ...
      if (this.consts.models.switchSingle.includes(device.model)) {
        ...
        *************/
        ...
        /***********/
      } else if (this.consts.diffuser.roller.includes(device.model)) {
        /********
        DIFFUSERS
        ********/
        accessory = devicesInHB.get(hbUUID) || this.addAccessory(device)
