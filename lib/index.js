...
class MerossPlatform {
      ...
      switch (key) {
        ...
        case 'singleDevices':
          if (Array.isArray(val) && val.length > 0) {
            val.forEach(x => {
              if (!x.serialNumber || !x.name || (x.deviceUrl && !x.model)) {
                logIgnoreItem(key)
                return
              }
    ...
    try {
      ...
      const context = {
        ...
        options: {
          ...
          lowBattThreshold: deviceConf.lowBattThreshold,
      ...
      if (this.consts.models.switchSingle.includes(device.model)) {
        ...
        ***************/
        ...
        for (const index in device.channels) {
          ...
          if (index > 0) {
            device.devName = device.channels[index].devName || devName + ' SW' + index
          }
        ...
        ***********/
        if (device.model === 'MSG200') {
          ...
          accessory.control = new (require('./device/garage-main'))(this, accessory, devicesInHB)
          ...
          for (const index in device.channels) {
            ...
            // The user has not hidden this channel
            const subAcc = devicesInHB.get(hbUUIDSub) || this.addAccessory(device)
            ...
            // Create the device type instance for this accessory
            subAcc.control = new (require('./device/garage-sub'))(this, subAcc, devicesInHB)
        ...
        subdevices.forEach(subdevice => {
          try {
            ...
            if (!this.consts.models.hubSub.includes(subdeviceObj.model)) {
              throw new Error(this.lang.sensorUnsupported)
            }
            ...
            switch (subdeviceObj.model) {
              case 'MS100':
                ...
                break
              case 'MTS100':
                subAcc.control = new (require('./device/hub-valve'))(this, subAcc)
                break
            }
            ...
            const eText = this.funcs.parseError(err, [this.lang.sensorUnsupported])
  ...
  async sendUpdate (accessory, toSend) {
    ...
    try {
      ...
      if (accessory.context.connection === 'cloud' && !this.config.hybridMode) {
      ...
      // Certain models aren't supported for local control, eventually put this list in constants
      if (accessory.context.model === 'MSH300') {
        throw new Error(this.lang.notSuppLocal)
      }

      // If cloud accessory, check we have an IP address
      if (accessory.context.connection === 'cloud' && !accessory.context.ipAddress) {
        throw new Error(this.lang.noIP)
      }

      // Obtain the IP address for the device
      ...
      res = await axios({
        ...
        data,
        responseType: 'json',
        timeout: toSend.method === 'GET' || accessory.context.connection === 'cloud' ? 4000 : 9000
      })
