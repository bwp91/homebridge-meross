        ...
        **********/
        ...
        subdevices.forEach(subdevice => {
          try {
            /*
              {
                subDeviceId: '120027A2C068',
                trueId: '27A2C068',
                subDeviceType: 'ms100',
                subDeviceSubType: 'un',
                subDeviceVendor: 'meross',
                subDeviceName: 'Some Sensor',
            ...
            */
            ...
            if (subdeviceObj.model !== 'MS100') {
              throw new Error('subdevice type is not supported yet')
            }

            const subAcc = devicesInHB.get(hbUUIDSub) || this.addAccessory(subdeviceObj)

            subAcc.context = { ...subAcc.context, ...{ subSerialNumber: subdevice.subDeviceId } }

            // Create the device type instance for this accessory
            subAcc.control = new (require('./device/hub-sensor'))(this, subAcc, devicesInHB)
        ...
        // Finally setup the main, hidden, accessory that will process the incoming updates
        accessory = this.addAccessory(device, true)
        accessory.context = { ...accessory.context, ...{ hidden: true } }
