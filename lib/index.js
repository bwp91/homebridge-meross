...
class MerossPlatform {
  ...
  async initialiseDevice (device) {
    try {
      ...
      const context = {
        ...
        connection: deviceConf.connection || (device.deviceUrl ? 'local' : 'default'),
        ...
        options: {
          ...
          capacityOverride: deviceConf.capacityOverride,
      ...
      if (context.connection === 'default') {
        context.connection = 'hybrid'
      }
  ...
  async sendUpdate (accessory, toSend) {
    ...
    try {
      ...
      if (accessory.context.connection === 'hybrid' && !accessory.context.ipAddress) {
