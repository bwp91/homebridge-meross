...
export default class {
  ...
  async login() {
    try {
      const nonce = platformFuncs.generateRandomString(16);
      const timestampMillis = Date.now();
      const loginParams = platformFuncs.encodeParams({
        email: this.username,
      ...
      if (Object.keys(res.data.data).length === 0) {
        ...
        if (res.data.info?.includes('password')) {
          if (this.base64Tried) {
  ...
  async getDevices() {
    try {
      ...
      if (
        ...
        || !platformFuncs.hasProperty(res.data, 'data')
