...
export default class {
  ...
  async login() {
    try {
      ...
      const res = await axios({
        url: `https://${this.domain}/v1/Auth/Login`,
        method: 'post',
        headers: {
          Authorization: 'Basic ',
          vender: 'Meross',
      ...
      if (!this.userkey) {
        this.log.debugWarn('%s: %s', platformLang.merossKey, this.key);
      }
      return {
        key: this.key,
        token: this.token,
  ...
  async getDevices() {
    try {
      ...
      res.data.data.forEach((device) => {
        // Don't initialise the device if ignored || (configured for local control and cloud control is disabled)
        if (this.ignoredDevices.includes(device.uuid) || (this.localUUIDs.includes(device.uuid) && !this.username)) {
  ...
  async getSubDevices(device) {
    try {
      ...
      const res = await axios({
        url: 'https://iotx.meross.com/v1/Hub/getSubDevices',
        method: 'post',
        headers: {
          Authorization: `Basic ${this.token}`,
          vender: 'Meross',
  ...
  async logout() {
    try {
      ...
      await axios({
        url: `https://${this.domain}/v1/Profile/logout`,
        method: 'post',
        headers: {
          Authorization: `Basic ${this.token}`,
          vender: 'Meross',
