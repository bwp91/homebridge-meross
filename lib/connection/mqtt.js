...
module.exports = class connectionMQTT {
  ...
  async sendUpdate (toSend) {
    ...
    if (this.status !== 'online') {
      ...
      return pTimeout.timeout(connectPromise.then(futureCommand), timeout)
    }
    ...
    return pTimeout.timeout(commandPromise, timeout)
  }

  async getSystemAllData () {
    return this.sendUpdate('GET', 'Appliance.System.All', {})
  }

  async getSystemDebug () {
    return this.sendUpdate('GET', 'Appliance.System.Debug', {})
  }

  async getSystemAbilities () {
    return this.sendUpdate('GET', 'Appliance.System.Ability', {})
  }

  async getSystemReport () {
    return this.sendUpdate('GET', 'Appliance.System.Report', {})
  }

  async getSystemRuntime () {
    return this.sendUpdate('GET', 'Appliance.System.Runtime', {})
  }

  async getSystemDNDMode () {
    return this.sendUpdate('GET', 'Appliance.System.DNDMode', {})
  }

  async setSystemDNDMode (onoff) {
    const payload = { DNDMode: { mode: onoff ? 1 : 0 } }
    return this.sendUpdate('SET', 'Appliance.System.DNDMode', payload)
  }

  async getOnlineStatus () {
    return this.sendUpdate('GET', 'Appliance.System.Online', {})
  }

  async getConfigWifiList () {
    return this.sendUpdate('GET', 'Appliance.Config.WifiList', {})
  }

  async getConfigTrace () {
    return this.sendUpdate('GET', 'Appliance.Config.Trace', {})
  }

  async getControlPowerConsumption () {
    return this.sendUpdate('GET', 'Appliance.Control.Consumption', {})
  }

  async getControlPowerConsumptionX () {
    return this.sendUpdate('GET', 'Appliance.Control.ConsumptionX', {})
  }

  async getControlElectricity () {
    return this.sendUpdate('GET', 'Appliance.Control.Electricity', {})
  }

  async getGarageDoorState (channel) {
    const payload = { state: { channel: channel } }
    return this.sendUpdate('GET', 'Appliance.GarageDoor.State', payload)
  }

  async controlToggle (onoff) {
    const payload = { toggle: { onoff: onoff ? 1 : 0 } }
    return this.sendUpdate('SET', 'Appliance.Control.Toggle', payload)
  }

  async controlToggleX (channel, onoff) {
    const payload = { togglex: { channel: channel, onoff: onoff ? 1 : 0 } }
    return this.sendUpdate('SET', 'Appliance.Control.ToggleX', payload)
  }

  async controlSpray (channel, mode) {
    const payload = { spray: { channel: channel, mode: mode || 0 } }
    return this.sendUpdate('SET', 'Appliance.Control.Spray', payload)
  }

  async controlGarageDoor (channel, open) {
    const payload = { state: { channel: channel, open: open ? 1 : 0, uuid: this.uuid } }
    return this.sendUpdate('SET', 'Appliance.GarageDoor.State', payload)
  }

  async controlRollerPosition (channel, position) {
    const payload = { position: { channel: channel, position } }
    return this.sendUpdate('SET', 'Appliance.RollerShutter.Position', payload)
  }

  async controlLight (light) {
    const payload = { light: light }
    return this.sendUpdate('SET', 'Appliance.Control.Light', payload)
  }

  async controlDiffusorSpray (type, channel, mode) {
    const payload = { spray: [{ channel: channel, mode: mode || 0, uuid: this.uuid }] }
    return this.sendUpdate('SET', 'Appliance.Control.Diffuser.Spray', payload)
  }

  async controlDiffusorLight (type, light) {
    light.uuid = this.uuid
    const payload = { light: [light] }
    return this.sendUpdate('SET', 'Appliance.Control.Diffuser.Light', payload)
  }

  async getHubBattery () {
    const payload = { battery: [] }
    return this.sendUpdate('GET', 'Appliance.Hub.Battery', payload)
  }

  async getMts100All (ids) {
    const payload = { all: [] }
    ids.forEach(id => payload.all.push({ id: id }))
    return this.sendUpdate('GET', 'Appliance.Hub.Mts100.All', payload)
  }

  async controlHubToggleX (subId, onoff) {
    const payload = { togglex: [{ id: subId, onoff: onoff ? 1 : 0 }] }
    return this.sendUpdate('SET', 'Appliance.Hub.ToggleX', payload)
  }

  async controlHubMts100Mode (subId, mode) {
    const payload = { mode: [{ id: subId, state: mode }] }
    return this.sendUpdate('SET', 'Appliance.Hub.Mts100.Mode', payload)
  }

  async controlHubMts100Temperature (subId, temp) {
    temp.id = subId
    const payload = { temperature: [temp] }
    return this.sendUpdate('SET', 'Appliance.Hub.Mts100.Temperature', payload)
  }
}
