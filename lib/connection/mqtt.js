...
module.exports = class connectionMQTT {
  ...
  connect () {
    ...
    this.client.on('message', (topic, msg) => {
      ...
      if (decodedMessage.header.from && !decodedMessage.header.from.includes(this.uuid)) {
        return
      }
      ...
      if (typeof resolveForThisMessage === 'function') {
        resolveForThisMessage({ data: decodedMessage })
        ...
        if (this.accessory.control && this.accessory.control.externalUpdate) {
          const namespace = decodedMessage.header ? decodedMessage.header.namespace : ''
          this.accessory.control.externalUpdate(namespace, decodedMessage)
        }
      }
    })
  ...
  async getGarageDoorState (channel) {
    ...
    return this.publishMessage('GET', 'Appliance.GarageDoor.State', payload)
  }
  ...
  async controlRollerPosition (channel, position) {
    ...
    return this.publishMessage('SET', 'Appliance.RollerShutter.Position', payload)
  }
