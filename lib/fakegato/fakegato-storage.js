...
class FakeGatoStorage {
  ...
  addWriter (service, params) {
    if (!this.addingWriter) {
      ...
      this.log('[%s] FG storage: addWriter().', service.accessoryName)
      const newWriter = {
        service,
        callback: params.callback,
        fileName: hostname + '_meross_' + service.accessoryName + '_persist.json'
      }
  ...
  write (params) {
    if (!this.writing) {
      ...
      writer.storageHandler.writeFile(fileLoc, params.data, 'utf8', err => {
        this.writing = false
        if (err) {
          this.log.warn('[%s] FGS write file [%s].', params.service.accessoryName, err)
          return
        }
        callBack(arguments)
      })
  ...
  read (params) {
    ...
    const callBack =
      typeof params.callback === 'function'
        ...
        ? writer.callback
        : () => {}
    this.log('FGS read file [%s].', path.join(writer.path, writer.fileName))
  ...
  remove (params) {
    ...
    const callBack =
      typeof params.callback === 'function'
        ...
        ? writer.callback
        : () => {}
    this.log('FGS delete file [%s].', path.join(writer.path, writer.fileName))
